...
export const Transactions = () => {
  ...
  const { MonthlySummary, Categories, Transactions, TransactionsPagination, setTransactionsPagination, setTransactions } = applicationStore();
  ...
  const handlePageChange = async (newPage: number) => {
    const response = await fetchTransactions(newPage, TransactionsPagination.limit);
  ...
  return (
    <>
      <div className={`space-y-8 ${displayForm ? "blur-sm" : ""} transition-all duration-200`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Transactions</h1>
            <p className="text-gray-600">Track and manage your financial activities</p>
          </div>
          <Button className="bg-finance-blue-accent hover:bg-finance-blue-accent/90 text-white rounded-xl px-6" onClick={() => setDisplayForm("transaction")}>
            Add Transaction
          </Button>
        </div>
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">-${MonthlySummary[0].total_expense}</p>
                </div>
            ...
            {Transactions.map((transaction) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center`} style={{ backgroundColor: Categories.find(c => c.id === transaction.category_id)?.color }}>
                    <span className="text-lg">
                      {Categories.find(c => c.id === transaction.category_id)?.icon}
                    </span>
                  </div>
                  <div>
                    <p className="font-semibold text-gray-900">{transaction.place}</p>
                    <p className="text-sm text-gray-500">{Categories.find(c => c.id === transaction.category_id)?.name} • {formatDate(transaction.transaction_date)}</p>
                  </div>
                </div>

                <div className="text-right">
                  <span className={`text-lg font-bold ${
                    Number(transaction.amount) > 0 ? 'text-finance-green-accent' : 'text-red-300'
                  }`}>
